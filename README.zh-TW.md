...
筆記。支援條件允許時可使用裝置端 Apple Speech，也可選擇明確下載的 Breeze ASR 26
台語離線模型；選用 Groq Cloud 前，App 會先顯示雲端處理告知並由使用者決定。
...
4. 在「設定 → 服務提供者」選擇 Apple Speech、Breeze ASR 26 或 Groq Cloud。Breeze
   需要你明確下載約 1.1 GB 的模型；Groq 功能需要你自己的憑證，並先確認雲端處理告知。
...
- **Breeze ASR 26：** 安裝模型後，音訊會留在這部 Mac，以 whisper.cpp 在程序內離線處理；
  針對台語最佳化，輸出主要是中文字，不等同正式台語羅馬字。
...
Apple Speech 通過能力檢查時可在裝置端運作；安裝 Breeze ASR 26 後也可離線辨識台語；
Groq 功能需要網路連線。
